...
import {
  ...
  ERROR_SET_NATION_NOT_REGISTERED,
    ...
    try {
      ...
      if (isEmpty(player)) {
        await bot.sendMessage(
          msg.chat.id,
          message(ERROR_SET_NATION_NOT_REGISTERED),
          {
        ...
        if (isEmpty(countryByCode) && isEmpty(countryByName)) {
          await bot.sendMessage(msg.chat.id, message(ERROR_SET_NATION_FORMAT), {
            parse_mode: config.getMessageParseMode(),
          });
        } else {
